...
class Projektseiten extends TwackComponent {
	...
	public function __construct($args) {
		...
		$this->addGlobalParameter(['projektseite' => $this->projektseite]);
		...
		if ($this->projektseite->titelbild) {
			...
			if ($this->titelbild->ext == 'svg') {
				$this->titelbild_html = $this->getProvider('BildProvider')->getBildTag(array(
				'bild' => $this->titelbild,
				...
				$this->titelbild_html =  $this->getProvider('BildProvider')->getBildTag([
				'bild' => $this->titelbild,
				'classes' => 'sv-inhalt bg-bild',
				'ausgabeAls' => 'bg-image',
				'normal' => array(
