...
if (!empty($this->portraits)) {
	...
	<div class="container-fluid">
		<div class="row portraits-reihe">
			<?php
			// Zuerst alle Portraits ausgeben, die zu dieser Rolle gehören:
			$counter = 1;
			foreach ($this->portraits->find('staffel_'.$this->staffel->id.'=1, root=1') as $portrait) {
				?>
			...
			// Portraits ausgeben, die zu Unterrollen gehören:
			if(!empty($this->unterrollen)){
				foreach($this->unterrollen as $rolle){
					$counter = 1;
					foreach ($this->portraits->find('staffel_'.$this->staffel->id.'_rolle_'.$rolle->id.'=1, root!=1') as $portrait) {
						?>
						<div class="col-6 col-sm-6 col-lg-4 col-xl-3 portrait-block">
							<?= $portrait->renderWithSubtitle($rolle->title); ?>
						</div>
