	...
	public function getRolle($rolleSeite = false, $anzeigeModus = false, $ebenen = 2) {
		...
		if ($anzeigeModus == 'als_block' || $anzeigeModus == 'als_block_mit_rollen') {
			...
			foreach ($this->portraitsContainer as $container) {
				$rollenAusgabe['portraits']->add(wire('pages')->find('template.name=portrait, sort=nachname, rollen.rolle.id='.$rolleSeite->id));
			}
			...
			if ($rollenAusgabe['staffeln']->count > 0) {
				...
				$rollenAusgabe['staffeln'] = $this->projektseite->get('template.name=staffeln_container')->children('id=' . $staffelnTmp->implode('|', 'id'));
			}
			...
			/*
			TODO: So gehts weiter:
			Auf neue Rollen-Portrait-Struktur umstellen. Danach Staffel-Ausgabe in Tabs, dann Animationen
			*/
		}
