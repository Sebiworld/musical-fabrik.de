...
/******/ 		return __webpack_require__.p + "js/chunk." + chunkId + "-" + {"0":"13cfac3a579ccc81897a","11":"4abeb0d00a75551766e3","56":"9918dd0b26f4ed28d523"}[chunkId] + ".min.js"
/******/ 	}
/******/
...
/* harmony import */ var _classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);
/* harmony import */ var _classes_AjaxAnfrage_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(64);
...
regeneratorRuntime.mark(function _callee() {
  var elemente, filterAbschicken, beitragKachelnAnzeigen, msnr, Masonry, progressively, logging;
  return regeneratorRuntime.wrap(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          elemente = document.querySelectorAll('.aktuelles-kacheln');
          ...
          */
          filterAbschicken = function filterAbschicken(kachelElement, grid, ajaxAnfrage) {
            ajaxAnfrage = ajaxAnfrage.clone();
            var getParams = ajaxAnfrage.getParams;
            var historyParams = Object.assign({}, getParams);
            delete historyParams.start;
            delete historyParams.offset; // Adressleiste umschreiben:

            window.history.pushState({}, document.title, '?' + ajaxAnfrage.objectToQueryString(historyParams));
            var vorhandeneIDs = [];
            var cards = grid.querySelectorAll('.beitrag-card');

            for (var index in cards) {
              var card = cards[index];

              if (_typeof(card) !== 'object' || !(card instanceof Element)) {
                continue;
              }
            ...
            getParams.action = 'getComponent', getParams.component = "AktuellesProvider";
            ...
            ajaxAnfrage.fetch({
              ...
              if (_typeof(data.beitraege) === 'object' && Array.isArray(data.beitraege)) {
              ...
              var container = grid.closest('.aktuelles-inhalte');

              if (_typeof(container) === 'object' && container instanceof Element) {
                var filtereinstellungen = container.querySelector('.filtereinstellungen');

                if (_typeof(filtereinstellungen) === 'object' && filtereinstellungen instanceof Element) {
                  // Schlagwort-Aktiv setzen
                  var aktiveSchlagwoerter = filtereinstellungen.querySelectorAll('.schlagwoerter .wort');

                  for (var i in aktiveSchlagwoerter) {
                    var s = aktiveSchlagwoerter[i];

                    if (_typeof(s) !== 'object' || !(s instanceof Element)) {
                      continue;
                    }

                    Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* removeClass */ "e"])(s, 'aktiv');
                  }

                  if (_typeof(getParams) === 'object' && getParams.schlagwoerter !== undefined) {
                    if (typeof getParams.schlagwoerter === 'string' && getParams.schlagwoerter.length > 0) {
                      Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* addClass */ "a"])(filtereinstellungen.querySelector('.schlagwoerter .wort[data-id="' + getParams.schlagwoerter + '"]'), 'aktiv');
                    } else if (_typeof(getParams.schlagwoerter) === 'object' && Array.isArray(getParams.schlagwoerter)) {
                      for (var _i in getParams.schlagwoerter) {
                        var schlagwortID = getParams.schlagwoerter[_i];
                        Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* addClass */ "a"])(filtereinstellungen.querySelector('.schlagwoerter .wort[data-id="' + schlagwortID + '"]'), 'aktiv');
                      }
                    }
                  }
                }
              ...
              if (data.gesamtAnzahl !== undefined && data.gesamtAnzahl > 0) {
                for (var _i2 in gesamtanzahlElemente) {
                  var gesamtanzahlElement = gesamtanzahlElemente[_i2];

                  if (_typeof(gesamtanzahlElement) !== 'object' || !(gesamtanzahlElement instanceof Element)) {
                    continue;
                  }
                ...
                for (var _i3 in gesamtanzahlElemente) {
                  var _gesamtanzahlElement = gesamtanzahlElemente[_i3];

                  if (_typeof(_gesamtanzahlElement) !== 'object' || !(_gesamtanzahlElement instanceof Element)) {
                    continue;
                  }

                  _gesamtanzahlElement.textContent = 'keine Beiträge';
                  _gesamtanzahlElement.style.display = 'none';
                }
              }

              var buttonGruppe = kachelElement.querySelector('.btn-group');

              if (_typeof(buttonGruppe) !== 'object' || !(buttonGruppe instanceof Element)) {
                grid.insertAdjacentHTML('beforebegin', '<div class="btn-group" role="group"></div>');
                buttonGruppe = kachelElement.querySelector('.btn-group');
              }

              var mehrButton = buttonGruppe.querySelector('[data-aktion="weitere_laden"]');

              if (_typeof(mehrButton) !== 'object' || !(mehrButton instanceof Element)) {
              ...
              if (data.hatMehr === true) {
                var offset = grid.querySelectorAll('.masonry-grid-item').length;
          ...
          beitragKachelnAnzeigen = function beitragKachelnAnzeigen(kachelElement, grid, beitragArray, abgleichen) {
            if (_typeof(grid) !== 'object' || !(grid instanceof Element)) {
              return false;
            }

            if (_typeof(beitragArray) !== 'object' || !Array.isArray(beitragArray)) {
              return false;
            }
            ...
            var gridItems = grid.querySelectorAll('.masonry-grid-item');

            if (beitragArray.length < 1 && (abgleichen !== false || gridItems.length < 1)) {
              // Alle Elemente aus dem Grid löschen:
              Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* removeElements */ "f"])(gridItems);
              grid.masonry('reloadItems').masonry('layout');
              progressively.drop();
              progressively.init({
                onLoad: function onLoad(elem) {
                  Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* trigger */ "g"])(elem, "bildGeladen");
                }
              }); // Meldung anzeigen:

              var keineErgebnisseElemente = kachelElement.querySelectorAll('.keine-ergebnisse');
              ...
              Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* trigger */ "g"])(grid, 'elementeGeaendert');
              return true;
            }

            Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* removeElements */ "f"])(kachelElement.querySelectorAll('.keine-ergebnisse')); // Über die vorhandenen Elemente wird iteriert. Wenn ein Element bereits vorhanden ist, muss es nicht gelöscht werden.

            var elementIterator = grid.querySelectorAll('.masonry-grid-item')[0];
            ...
            if (abgleichen === false) {
              if (beitragArray.length < 1) {
                ...
                var fund = grid.querySelector('.beitrag-card[data-id="' + beitragArray[0].id + '"]');

                if (_typeof(fund) === 'object' && fund instanceof Element) {
                  // Das erste Element der neuen Kacheln existiert schon.
                  elementIterator = fund.parentNode;
                } else {
                  elementIterator = undefined; // const gridElemente = grid.querySelectorAll('.masonry-grid-item');
                  // elementIterator = gridElemente[gridElemente.length - 1].nextElementSibling;
                }
              }
            }

            for (var index in beitragArray) {
              Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* trigger */ "g"])(grid, 'elementeGeaendert');
              var beitrag = beitragArray[index];

              if (logging) {
                console.log("Nächstes Element: ", beitrag, index);
              }

              if (_typeof(beitrag) !== 'object' || typeof beitrag.html !== 'string' || beitrag.html.length < 1 || beitrag.id === undefined) {
              ...
              var _fund = grid.querySelector('.beitrag-card[data-id="' + beitrag.id + '"]');

              if (_typeof(_fund) === 'object' && _fund instanceof Element) {
                ...
                while (elementIterator) {
                  ...
                  if (_typeof(elementIterator) !== 'object' || !(elementIterator instanceof Element)) {
                    break;
                  }
                  ...
                  var oldElement = elementIterator;
                  elementIterator = elementIterator.nextElementSibling;
                  oldElement.parentNode.removeChild(oldElement);
                }

                elementIterator = _fund;
                elementIterator = elementIterator.nextElementSibling;
                continue;
            ...
            if (abgleichen !== false) {
              while (elementIterator) {
                ...
                var _oldElement = elementIterator;
                elementIterator = elementIterator.nextElementSibling;

                _oldElement.parentNode.removeChild(_oldElement);
              }
            }
          ...
          _context.next = 6;
          return Promise.all(/* import() */[__webpack_require__.e(0), __webpack_require__.e(56)]).then(__webpack_require__.bind(null, 289));

        case 6:
          msnr = _context.sent;
          _context.next = 9;
          return __webpack_require__.e(/* import() | masonry-script */ 0).then(__webpack_require__.t.bind(null, 122, 7));

        case 9:
          Masonry = _context.sent;
          _context.next = 12;
          return __webpack_require__.e(/* import() | bildtools */ 11).then(__webpack_require__.t.bind(null, 296, 7));

        case 12:
          progressively = _context.sent;
          logging = false;
          Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* ready */ "d"])(function () {
            var _loop = function _loop(index) {
              var kachelElement = elemente[index];

              if (_typeof(kachelElement) !== 'object' || !(kachelElement instanceof Element)) {
                return "continue";
              }

              var grid = kachelElement.querySelector('.masonry-grid');
              ...
              var ajaxAnfrage = new _classes_AjaxAnfrage_js__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"]();
              var mehrButton = kachelElement.querySelector('[data-aktion="weitere_laden"]');

              if (_typeof(mehrButton) === 'object' && mehrButton instanceof Element) {
                mehrButton.addEventListener("click", Object(lodash__WEBPACK_IMPORTED_MODULE_2__["debounce"])(function (event) {
                  event.preventDefault();
                  var getParameter = ajaxAnfrage.getParams;
                  var offset = mehrButton.getAttribute('data-offset');
              ...
              if (_typeof(container) === 'object' && container instanceof Element) {
                ...
                if (_typeof(filtereinstellungen) === 'object' && filtereinstellungen instanceof Element) {
                  ...
                  var suchenBtn = filtereinstellungen.querySelector('.btn[name="suchen"]');
                  ...
                  var schlagwoerter = filtereinstellungen.querySelectorAll('.schlagwoerter .wort');

                  if (schlagwoerter.length > 0) {
                    var _loop2 = function _loop2(i) {
                      var schlagwort = schlagwoerter[i];

                      if (_typeof(schlagwort) !== 'object' || !(schlagwort instanceof Element)) {
                        return "continue";
                      }

                      schlagwort.addEventListener("click", function (event) {
                        event.preventDefault();
                        var schlagwortID = "" + schlagwort.getAttribute('data-id');
                        ...
                        if (_typeof(getParameter.schlagwoerter) !== 'object' || !Array.isArray(getParameter.schlagwoerter)) {
                          getParameter.schlagwoerter = [];
                        }

                        if (!Object(_classes_hilfsfunktionen_js__WEBPACK_IMPORTED_MODULE_0__[/* hasClass */ "b"])(schlagwort, 'aktiv')) {
                          getParameter.schlagwoerter.push(schlagwortID);
                        } else {
                          Object(lodash__WEBPACK_IMPORTED_MODULE_2__["remove"])(getParameter.schlagwoerter, function (s) {
                            return s == schlagwortID;
                          });
                        }

                        getParameter.schlagwoerter = Object(lodash__WEBPACK_IMPORTED_MODULE_2__["uniq"])(getParameter.schlagwoerter);
                  ...
                  var freitextsucheElemente = filtereinstellungen.querySelectorAll('input[name="freitextsuche"]');

                  if (freitextsucheElemente.length > 0) {
                    var _loop3 = function _loop3(i) {
                      var freitextsuche = freitextsucheElemente[i];

                      if (_typeof(freitextsuche) !== 'object' || !(freitextsuche instanceof Element)) {
                        return "continue";
                      }

                      freitextsuche.addEventListener("keydown", Object(lodash__WEBPACK_IMPORTED_MODULE_2__["debounce"])(function (event) {
                        event.preventDefault();
                        var getParameter = ajaxAnfrage.getParams;
                        delete getParameter.start;
                        var eingabe = freitextsuche.value;
                    ...
                    for (var i in freitextsucheElemente) {
                      var _ret3 = _loop3(i);
