...
class SebiMusicalFabrikFunctions extends WireData implements Module {
	...
	protected function initProjectPage(Page $projectPage) {
		if ($projectPage->numChildren > 0) {
		...
		$page->parent    = $projectPage;
		$page->name      = 'tickets-und-infos';
		$page->title     = 'Tickets und Infos';
		$page->published = true;
		$page->save();
		...
		$seasonsContainer = $projectPage->children('template.name=seasons_container')->first;
		...
		$viewPermission = $this->wire(new Permission());
		$viewPermission->name      = 'project-' . $projectPage->name . '-view';
		$viewPermission->title     = 'Kann interne "' . $projectPage->title . '" Inhalte sehen';
		$viewPermission->api_visible = true;
		$viewPermission->save();

		$managePermission = $this->wire(new Permission());
		$managePermission->name      = 'project-' . $projectPage->name . '-manage';
		$managePermission->title     = 'Darf "' . $projectPage->title . '" Inhalte verwalten';
		$managePermission->api_visible = true;
		$managePermission->save();

		$projectRole = $this->wire(new Role());
		$projectRole->name      = 'project-' . $projectPage->name;
		$projectRole->title     = 'Mitwirkende "' . $projectPage->title . '"';
		$projectRole->api_visible = true;
		$projectRole->addPermission('project-' . $projectPage->name . '-view');
		$projectRole->save();

		$projectAdminRole = $this->wire(new Role());
		$projectAdminRole->name      = 'project-' . $projectPage->name . '-admin';
		$projectAdminRole->title     = 'Admin "' . $projectPage->title . '"';
		$projectAdminRole->api_visible = true;
		$projectAdminRole->addPermission('project-' . $projectPage->name . '-view');
		$projectAdminRole->addPermission('project-' . $projectPage->name . '-manage');
		$projectAdminRole->save();
	}
