...
class SebiMusicalFabrikFunctions extends WireData implements Module {
	...
	public function init() {
		...
		$this->addHookAfter('SeoMaestro::sitemapItems', $this, 'hookSitemapItems');
	}

	public function hookSitemapItems(HookEvent $event) {
		$items = $event->return;

		if (is_array($items) && !empty($items)) {
			foreach ($items as &$item) {
				$item['loc'] =  AppApi::getHttpUrlRelativeToRoot($item['loc']);
			}
		}
