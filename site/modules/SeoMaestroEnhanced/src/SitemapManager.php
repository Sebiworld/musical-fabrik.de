...
class SitemapManager extends WireData
{
     ...
     * @throws SeoMaestroEnhancedException
     *
     * @return bool
     */
    public function generate($sitemapPath)
    {
        ...
        if (!is_writeable($parentDir)) {
            throw new SeoMaestroEnhancedException(
    ...
    {
        $templates = $this->getTemplatesWithSeoMaestroEnhancedField();
    ...
    private function getTemplatesWithSeoMaestroField()
    {
        ...
        foreach ($this->wire('templates') as $template) {
            $fields = $template->fields->find('type=FieldtypeSeoMaestroEnhanced');
            if (!$fields->count()) {
                continue;
            }
    ...
    private function buildSitemapItemsFromPage(Page $page, SitemapSeoData $sitemapData)
    {
        ...
        if ($languageSupport && $languageSupportPageNames) {
            foreach ($this->wire('languages') as $language) {
                ...
                $loc = $this->get('baseUrl') ? $this->get('baseUrl') . \ProcessWire\AppApi::getUrlRelativeToRoot($page->localUrl($language)) : \ProcessWire\AppApi::getHttpUrlRelativeToRoot($page->localHttpUrl($language));
        ...
        $item = (new SitemapItem())
            ...
            ->set('lastmod', $this->getLastMod($page))
            ->set('changefreq', $sitemapData->changeFrequency)
            ->set('loc', $this->get('baseUrl') ? $this->get('baseUrl') . \ProcessWire\AppApi::getUrlRelativeToRoot($page->url) : \ProcessWire\AppApi::getHttpUrlRelativeToRoot($page->httpUrl));
